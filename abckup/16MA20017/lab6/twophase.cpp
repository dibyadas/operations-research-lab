...
Collect simplex_p1(vector<EQN> eqns, EQN opt, int num_eqns, int num_vars, vector<float> last_row, vector<int> artificial_index){
	...
	map<int, int> track_af_vars;
	for( auto index : artificial_index){
		track_af_vars[index] = -1;
	}
	...
	while(1){
		...
		//////////////////////////////// check if artifical variables are zero ////////////////////////////////////////////////////////////
		int flag = 1;
		for(auto const& x : track_af_vars){
		...
		if (flag == -1){
			cout << "artifical vars not yet zero, keep going ";
		}
		else{
			cout << "artifical vars have become zero, continue to phase 2";
			Collect c1;
			c1.mat = mat;
			c1.map_non_basic = map_non_basic;
			return c1;
		}
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void twophase(vector<EQN> eqns, int num_eqns, int num_vars){
	vector<int> artificial_index;
	int extra = 0;
	for(int index = 0; index <= num_eqns; index++){
		if (eqns[index].sign == 2 || eqns[index].sign == 3)
		{
			// eqn.num_vars++;
			extra++;
			artificial_index.push_back(index);
			// eqn.coeffs.push_back(-1);
		}
	}
	// cout << "extra " << extra << "\n";
	for(auto v: artificial_index){
		cout << v << "  f  \n"; 
	}
	...
	// cout << endl;
	// for(auto eqn_index : artificial_index){
	// 	for (int i = 0; i < num_vars + extra; ++i)
	// 	{
	// 		cout << eqns[eqn_index].coeffs[i] << "  ";
	// 	}
	// 	cout << endl;
	// }

	// prep for phase 1
	vector<float> p1_coeff;
	for (int j = 0; j < num_vars + extra; ++j)
	{
		...
		for(auto eqn_index : artificial_index)
		{	
			sum += eqns[eqn_index].coeffs[j];
		}
		// cout << "sum " << sum;
		p1_coeff.push_back( - sum);
	}

	float sum = 0;
	for( auto eqn_index : artificial_index)
	{	
		sum += eqns[eqn_index].prep_gauss()[num_vars + extra ];
	}
	p1_coeff.push_back( - sum);
	...
	cout << endl;
	// return;
	Collect phase1 = simplex_p1(eqns, opt , num_eqns, num_vars + extra, p1_coeff, artificial_index);
	float** new_mat = phase1.mat;
	map<int, string> map_non_basic = phase1.map_non_basic;
	vector<int> col_to_zero;

	for(auto index : artificial_index){
		for(auto const& x : map_non_basic){
			if( ("z" + to_string(index+1)) == x.second ){
				// cout << x.second << "  " << x.first << "  ff  \n"; 
				col_to_zero.push_back(x.first);
			}
		}
	}

	for (int col_index : col_to_zero)
	{
		for(int i = 0; i < num_eqns + 1; i++){
			new_mat[i][col_index] = 0;
		}
	}
	...
	for (int i = 0; i < num_eqns + 1; ++i)
	{
		for (int j = 0; j < num_vars + extra + 1; ++j)
		{
			cout << new_mat[i][j] << "      ";
		}
		cout << endl;
...
int main(int argc, char const *argv[])
 {
 	...
 	twophase(eqns, num_eqns, num_vars);
